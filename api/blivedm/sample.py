...
class MyHandler(blivedm.BaseHandler):
    ...
    async def _on_danmaku(self, client: blivedm.BLiveClient, message: blivedm.DanmakuMessage):
        ...
        if message.msg.find(keyword) != -1:
            # 获取当前时间
            current_time = int(time.time())
            print(current_time) # 1631186249
            # 转换为localtime
            localtime = time.localtime(current_time)
            dt = time.strftime('%Y-%m-%dT%H:%M:%S GMT+0800', localtime)
            content = {"roomid": client.room_id, "username": message.uname, "msg": message.msg, "time": dt}
            with open('./data/'+str(client.room_id)+'.json', 'w', encoding='utf-8') as f:

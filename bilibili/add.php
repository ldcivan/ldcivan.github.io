...
<?php
    ...
    if(!empty($_POST['new_uid'])){
        ...
        function check_url($url)
        {
            ...
            # 获取状态码赋值
            preg_match('/"code":(.*?),/',$jsondata,$code);
            return $code[1];
        }
        //echo(check_url($url));
        if(check_url($url) != 0) exit('<script>alert("无法拉取该uid对应页面！请检查uid或稍后再试……(code:'.check_url($url).')");window.open("/bilibili", "_self");</script>');
        ...
        else exit('<script>alert("uid已在观测列表！");window.open("/bilibili?uid='.$new_uid.'", "_self");</script>');
    }
?>
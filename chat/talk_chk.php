<?php
	...
	if($_SESSION['user'] != ''){
		if($_GET['action'] == 'send'){ 
			...
			$cont = $_GET['cont'];						//说话内容
			$now = date('H:i:s');
			$newline = '[<font color=blue>'.$user.'</font>]<font color=red>'.$_GET['face'].'</font>对[<font color=green>'.$_GET['obt'].'</font>]说：<font color='.$_GET['color'].'>'.$cont.'</font>&nbsp;[<font color=brown>'.$now.'(UTC+8)</font>]';
			$tmpsend = 'priv/'.$user;
			$tmpto = 'priv/'.$_GET['obt'];
			if($_GET['obt'] == '所有人'){
				...
				$newline = '[<font color="pink">系统公告</font>]：<font color="red">您没有朋友对吗（</font>&nbsp;[<font color=brown>'.date('H:i:s').'</font>]';

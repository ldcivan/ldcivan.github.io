...
<body>
  ...
  if ($lbtype == 'year') {
    $title = "年";
    $cond = "DATE_SUB(CURDATE(), INTERVAL 1 YEAR) <= date(time)";
  }
  if ($lbtype == 'all') {
    $title = "总";
    $cond = "DATE_SUB(CURDATE(), INTERVAL 99 YEAR) <= date(time)";
  }
  ?>
  ...
  <div class="page-header text-center">
    ...
    <a href="?lbtype=year<?php echo $CurrentUser ? "&name=" . $CurrentUser : "" ?>"><button type="button" class="btn btn-outline-secondary btn-sm">年榜</button></a>

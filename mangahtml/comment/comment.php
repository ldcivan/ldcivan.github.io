<?php
    ...
    $url = $_SERVER['HTTP_REFERER']; // 获取请求来源页面的URL
    $page = basename(parse_url($url, PHP_URL_PATH)); // 获取最后一个斜杠后的内容
    $subtime=date("Y-m-d H:i:s");
    $fileName = './comment.txt'; //Open your .txt file
    //ftruncate($file, 0); //Clear the file to 0bit
    $location = explode('|',$region)[2];
    $content = "User_Name ". $title. PHP_EOL. " Comment  ". $gain. PHP_EOL. "  Time.   ". $subtime. "(UTC+8) 发自". $location. PHP_EOL. "  Page.   ". $page. PHP_EOL. "---------". PHP_EOL;
